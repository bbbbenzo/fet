...
<i>Отправьте /next, чтобы начать поиск</i>
"""

    @staticmethod
    def search_stopped() -> str:
        """Сообщение об остановке поиска"""
        return f"""
{design.format_header("⛔️ Поиск остановлен")}
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    # Отправляем приветствие без загрузочного сообщения
...
async def cmd_stop_button(message: Message, state: FSMContext):
    ...
    if partner_id:
        # Есть активный диалог - завершаем
...
async def cmd_stop_command(message: Message, state: FSMContext):
    """Универсальная команда остановки (поиска или диалога)"""
    current_state = await state.get_state()
    user_id = message.from_user.id

    # Проверяем, есть ли активный партнер
    partner_id = await db.get_active_partner(user_id)
    ...
    if current_state == ChatStates.searching:
        ...
        # Завершаем диалог (когда есть активный партнер)
        ...
        if partner_id:
            try:
                await bot.send_message(
                    chat_id=partner_id,
                    text="<i>Собеседник вышел из диалога</i>",
                    reply_markup=create_keyboard_main(),
                    parse_mode="HTML"
                )
            except Exception as e:
    ...
    elif current_state == ChatStates.chatting and not partner_id:
        # Если в состоянии chatting, но партнера нет (ошибка состояния)
        await state.set_state(ChatStates.main)
        ...
        # Если не в поиске и не в чате (состояние main или другое)
        await message.answer(
            "<i>У вас нет собеседника 😐\n\n"
            "Отправьте /next, чтобы начать поиск</i>",
            parse_mode="HTML",
...
async def stop_handler(message: Message, state: FSMContext):
    """Общий обработчик завершения диалога и остановки поиска"""
    user = await db.get_user_by_telegram_id(message.from_user.id)
    ...
    # Если в состоянии поиска - останавливаем поиск
    if current_state == ChatStates.searching:
        await db.leave_search_queue(message.from_user.id)
        await state.set_state(ChatStates.main)
    ...
    # Если в состоянии чата - завершаем диалог
    partner_id = await db.end_session(message.from_user.id)
